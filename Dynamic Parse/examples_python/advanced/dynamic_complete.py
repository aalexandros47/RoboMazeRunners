...
This example demonstrates how to use lexer='dynamic_complete' and ambiguity='explicit'.

In cases where data is highly ambiguous or 'broken,' using parser='earley' and lexer='dynamic_complete' allows Lark to parse almost anything, as long as there is a valid way to generate it from the grammar, including looking 'into' the regexes.

This example shows how to parse a JSON input where the quotes have been replaced by underscores: {_foo_:{}, _bar_: [], _baz_: __}. Note that underscores might still appear inside strings, so a potentially valid interpretation of the above is: {"foo_:{}, _bar": [], "baz": ""}.
