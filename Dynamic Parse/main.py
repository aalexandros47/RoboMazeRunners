...
Input Handling: The program expects two command-line arguments: a filename and a method (TT for truth table, FC for forward chaining, BC for backward chaining).
File Reading: Reads logical statements from a file specified by the filename.
Symbol Extraction: Identifies unique symbols and sentences from the provided logical statements.
Knowledge Base Creation: Parses the sentences into a logical format that can be processed for truth evaluation.
Query Parsing: Transforms a query string into a logical sentence.
Method Execution: Depending on the method specified (truth table, forward chaining, or backward chaining), the appropriate logic is applied to derive conclusions based on the 
